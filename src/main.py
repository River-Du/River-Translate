...
HELP_TEXT = (
    ...
    "【项目地址】\n"
    "https://github.com/river-du/river-translate\n\n"
    "【快捷键】\n"
    "Enter ：翻译\n"
    "Ctrl/Shift + Enter ：换行\n"
    "Escape：清空输入输出；翻译中按Escape终止\n\n"
    "【语言】\n"
    "源语言可设为「自动检测」。\n"
    "目标语言可设为「自动中英」：根据输入首个字符自动选择中文或英语。\n"
    "当指定源语言和目标语言时，可交换两者。\n\n"
    "【翻译引擎】\n"
    "「谷歌翻译」: 免费接口无需配置，开箱即用。\n"
    "「百度翻译」: 国内可直接使用，需在设置中配置。\n"
    "「DeepL」: 翻译质量高，需在设置中配置。\n"
    "「自定义AI」: 支持OpenAI兼容接口如DeepSeek等，需在设置中配置，可填写「领域/风格」来调整翻译风格。\n\n"
    "【可选功能】\n"
    "剪贴板翻译 ：检测到新剪贴板内容时自动翻译。\n"
    "自动翻译 ：停止输入约 1 秒后自动翻译。\n"
    ...
    "恢复默认设置 : 删除user_data/config.json后重启应用。\n"
    "配置和历史保存在user_data目录，可能包含API_Key，请勿分享。\n"
)
